...
class financeModel {
    ...
    public function loadSummary($id_event) {
        ...
        $query = "SELECT count(DISTINCT a.id) as 'num_pacotes', a.status as 'status_pacote', sum(c.valor) as 'valor_total' FROM pacote a "
                ...
                . "WHERE id_evento = $id_event AND c.status != 0 GROUP BY a.status ORDER BY a.status DESC";
        $summary["pacotes"] = $this->conn->freeQuery($query, true, PDO::FETCH_ASSOC);
        ...
        foreach($summary["pacotes"] as $index => $pacotes) {
            ...
            $query = "SELECT count(a.id) as 'num_parcelas', sum(a.valor) as 'valor_parcelas', a.status as 'status_parcela' FROM parcela_pacote a "
                   ...
                   . " GROUP BY a.status";
            $summary["pacotes"][$index]["parcelas"] = $this->conn->freeQuery($query, true, PDO::FETCH_ASSOC);   
        }
